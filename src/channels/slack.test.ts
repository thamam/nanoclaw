...
vi.mock('@slack/bolt', () => ({
  App: class MockApp {
    ...
    client = {
      auth: {
        test: vi.fn().mockResolvedValue({ user_id: 'U_BOT_123', bot_id: 'B_BOT_OUR' }),
      },
      chat: {
...
describe('SlackChannel', () => {
  ...
  describe('message handling', () => {
    ...
    it('detects own bot messages by matching bot_id', async () => {
      const opts = createTestOpts();
      const channel = new SlackChannel(opts);
      await channel.connect(); // sets botId to 'B_BOT_OUR'
      ...
      // Matches our bot_id so is_from_me should be true
      expect(opts.onMessage).toHaveBeenCalledWith(
        'slack:C0123456789',
        expect.objectContaining({
          is_from_me: true,
    ...
    it('treats messages from other bots as not from me', async () => {
      const opts = createTestOpts();
      const channel = new SlackChannel(opts);
      await channel.connect();
      ...
      const event = {
        ...createMessageEvent({
          subtype: 'bot_message',
          botId: 'B_OTHER_BOT',
          text: 'Nook says hello',
        }),
        bot_profile: { name: 'Nook' },
        username: 'nook',
      };
      await triggerMessageEvent(event);
    ...
    it('falls back to username then bot_id for other bots with no profile', async () => {
      const opts = createTestOpts();
      const channel = new SlackChannel(opts);
      await channel.connect();
