  ...
  private setupEventHandlers(): void {
    ...
    this.app.event('message', async ({ event }) => {
      ...
      if (isFromMe) {
        ...
        // Another bot: try to get its display name.
        // @slack/types doesn't expose bot_profile/username on BotMessageEvent,
        // but the Slack API does include them — cast to access at runtime.
        const botMsg = msg as BotMessageEvent & {
          bot_profile?: { name?: string };
          username?: string;
        };
        senderName =
          botMsg.bot_profile?.name ||
          botMsg.username ||
          msg.bot_id ||
          'unknown_bot';
      } else {
