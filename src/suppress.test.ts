...
describe('shouldSuppressOutput', () => {
  ...
  it('suppresses "All bots healthy" variants', () => {
    expect(shouldSuppressOutput('All bots healthy — no output needed.')).toBe(
      true,
    );
    expect(
      shouldSuppressOutput('All bots are healthy — no output needed.'),
    ).toBe(true);
    expect(shouldSuppressOutput('All bots healthy — nothing to report.')).toBe(
      true,
    );
    expect(shouldSuppressOutput('All bot healthy — no output needed.')).toBe(
      true,
    );
  });
  ...
  it('does NOT suppress real alerts', () => {
    ...
    expect(
      shouldSuppressOutput('🔴 CRITICAL — DB has been down for 10 minutes'),
    ).toBe(false);
    expect(shouldSuppressOutput('Nook is back (was down)')).toBe(false);
    expect(
      shouldSuppressOutput('All bots healthy but there is a trend to watch'),
    ).toBe(false);
  });

  it('does NOT suppress multi-line messages', () => {
    expect(shouldSuppressOutput('All bots healthy\nBut also check this')).toBe(
      false,
    );
  });
