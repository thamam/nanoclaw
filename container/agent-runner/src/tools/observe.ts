...
export interface BotStatusJson {
  bot: string;
  state: 'running' | 'stopped' | 'restarting' | 'not_found' | 'unknown';
...
/**
 * Parse Docker's "running for" string into approximate seconds.
 */
function parseUptimeToSeconds(runningFor: string): number | null {
  if (!runningFor) return null;
  const parts = runningFor.toLowerCase().trim();
  let total = 0;
  const dayMatch = parts.match(/(\d+)\s*day/);
  const hourMatch = parts.match(/(\d+)\s*hour/);
  const minMatch = parts.match(/(\d+)\s*minute/);
  const secMatch = parts.match(/(\d+)\s*second/);
  ...
  // Handle "About an hour" / "About a minute" patterns
  if (parts.includes('about an hour')) total = 3600;
  if (parts.includes('about a minute')) total = 60;
  return total || null;
}
...
export async function botStatus(
  ...
  // JSON format — structured output for watcher consumption
  if (format === 'json') {
    const checkedAt = new Date().toISOString();
    if (dockerResult.exitCode !== 0) {
      return JSON.stringify({
        bot: bot.toLowerCase(), state: 'unknown', uptime_seconds: null,
        status_string: `SSH/Docker error: ${dockerResult.stderr.trim()}`,
        ssh_ok: false, checked_at: checkedAt,
      } satisfies BotStatusJson);
    }
    const raw = dockerResult.stdout.trim();
    if (!raw) {
      return JSON.stringify({
        bot: bot.toLowerCase(), state: 'not_found', uptime_seconds: null,
        status_string: 'Container not found or not running',
        ssh_ok: true, checked_at: checkedAt,
      } satisfies BotStatusJson);
    }
    const [statusStr, stateStr, runningFor] = raw.split('\t');
    const mappedState = stateStr === 'running' ? 'running'
      : stateStr === 'restarting' ? 'restarting' : 'stopped';
    return JSON.stringify({
      bot: bot.toLowerCase(), state: mappedState,
      uptime_seconds: parseUptimeToSeconds(runningFor ?? ''),
      status_string: statusStr ?? 'unknown',
      ssh_ok: true, checked_at: checkedAt,
    } satisfies BotStatusJson);
  }

  // Human format — original behavior
